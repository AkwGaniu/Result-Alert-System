...
// http://localhost/smsRequest/result_ussd_modified.php?MSISDN=0713038301&USSD_STRING=&serviceCode=*144#
        ...
        if(isset($level[0]) && $level[0]!="" && !isset($level[1])){
            ...
            if(mysqli_num_rows($resultStud) > 0){
                ...
                $response="END \r\nIncorrect matric number or matric number does not exist\nPlease check your matric number and try again.";
            }
        ...
        if(isset($level[2]) && $level[2]!="" && !isset($level[3])){
            ...
            if($level[1] == 1 && $level[2] == 1){
                ...
                $result_table = "exam_score";
            }
            elseif($level[1] == 1 && $level[2] == 2){
                $course_table = "";
                $result_table = "";
            }
            elseif($level[1] == 2 && $level[2] == 1){
                $course_table = "";
                $result_table = "";
            }
            elseif($level[1] == 2 && $level[2] == 2){
                $course_table = "";
                $result_table = "";
            }else{
                $response="CON \r\n Please select semester\n1.  FIRST SEMESTER\n2.  SECOND SEMESTER";
        ...
        if ($coure_table != "" && $result_table != "") {
            $response="CON \r\n E don set.";

            $sql = "SELECT $result_table.courseCode, $result_table.score, $course_table.courseUnit FROM $course_table, $result_table
            WHERE $result_table.matricNum = $level[0] AND $course_table.courseCode = $result_table.courseCode ";
